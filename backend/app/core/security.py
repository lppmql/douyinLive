...
_MEDIA_PATH_PATTERN = re.compile(
    r"^/api/v1/(?:live-sessions/\d+/(?:avatar|video|stream|playback|comments/\d+/avatar)|clip/clips/\d+/(?:video|cover|subtitle|subtitle\.srt))$"
)
...
def create_media_access_token(user_id: int) -> str:
    ...
    expire = datetime.now(timezone.utc) + timedelta(
        minutes=settings.MEDIA_ACCESS_TOKEN_EXPIRE_MINUTES
    )
...
def get_current_user(
    ...
    if (
        ...
        and _MEDIA_PATH_PATTERN.fullmatch(request.url.path)
    ):

...
def serialize_scraper_task(task: ScraperTask) -> dict[str, Any]:
    ...
    return {
        ...
        "can_retry": status in {TaskStatus.FAILED, TaskStatus.CANCELLED} and task.is_retryable is not False,
...
def serialize_asr_task(
    ...
    return {
        ...
        "error_code": None,
        "failure_stage": None,
        "is_retryable": status in {TaskStatus.FAILED, TaskStatus.CANCELLED},
        "trace_id": task.trace_id,
        "worker_id": task.worker_id,
        "heartbeat_at": task.heartbeat_at,
        "retry_count": int(task.retry_count or 0),

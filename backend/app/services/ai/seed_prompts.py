...
def seed_prompts(db: Session) -> None:
    """仅在提示词表为空时初始化当前领域版本。"""
    if db.query(PromptTemplate).first():
        return
    db.add_all(PromptTemplate(**prompt) for prompt in INITIAL_PROMPTS)
    db.commit()
    print(f"已初始化 {len(INITIAL_PROMPTS)} 条零食店直播领域提示词")

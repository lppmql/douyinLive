...
class AsrTask(Base, TimestampMixin):
    ...
    __table_args__ = (
        ...
        Index(
            "idx_asr_tasks_source_status_priority", "queue_source", "status", "priority"
        ),
    ...
    stream_id = Column(
        Integer, ForeignKey("stream_sources.id"), nullable=True, comment="关联流源"
    )
    status = Column(
        String(20), default="queued", comment="状态: queued/processing/completed/failed"
    )
    task_type = Column(
        String(20), default="realtime", comment="任务类型: realtime/offline"
    )
    queue_source = Column(
        String(20),
        nullable=False,
    ...
    cancel_requested_at = Column(
        DateTime, nullable=True, comment="用户请求安全停止转写的时间"
    )
    ...
    priority = Column(
        Integer, nullable=False, default=50, comment="优先级，数值越小越优先"
    )
    ...
    postprocess_completed_at = Column(
        DateTime, nullable=True, comment="知识库同步完成时间"
    )
    postprocess_error = Column(Text, nullable=True, comment="后处理失败或降级原因")
    postprocess_attempt_count = Column(

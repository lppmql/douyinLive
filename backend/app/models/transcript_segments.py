...
class TranscriptSegment(Base, TimestampMixin):
    ...
    asr_chunk_id = Column(
        BigInteger,
        ForeignKey("asr_audio_chunks.id"),
        nullable=True,
        comment="关联ASR音频分片",
    )
    ...
    raw_text_content = Column(Text, nullable=True, comment="FunASR 原始话术（纠错前）")
    word_timestamps_json = Column(
        JSON,
        nullable=True,
    ...
    segment_type = Column(
        String(50), nullable=True, comment="话术类型：开场/产品介绍/互动/留资引导/结束"
    )
    asr_status = Column(
        String(20),
        default="pending",
        comment="转写状态：pending/processing/completed/failed",
    )

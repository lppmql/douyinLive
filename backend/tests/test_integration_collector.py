...
class TestCollectorStatus:
    ...
    def test_status_returns_ok_with_empty_db(self, client):
    ...
    def test_status_returns_connected_with_account(self, client, db):
        """有账号 → connected=true"""
...
class TestCollectorAccounts:
    ...
    def test_list_accounts_empty(self, client):
        """空列表 → 200 + []"""
        resp = client.get("/api/v1/collector/accounts")
        assert resp.status_code == 200
        assert resp.json() == []
    ...
    def test_create_and_list_account(self, client):
        ...
        # 列表
        list_resp = client.get("/api/v1/collector/accounts")
        assert list_resp.status_code == 200
        assert len(list_resp.json()) == 1
        assert list_resp.json()[0]["account_name"] == "新账号"

    def test_get_single_account(self, client, db):
        """获取单个账号详情"""
        from app.models.scraper_accounts import ScraperAccount
        ...
        resp = client.get(f"/api/v1/collector/accounts/{account.id}")
        assert resp.status_code == 200
        assert resp.json()["account_name"] == "详情账号"

    def test_get_nonexistent_account_returns_404(self, client):
        """不存在的账号 → 404"""
        resp = client.get("/api/v1/collector/accounts/99999")
    ...
    def test_delete_account(self, client, db):
        ...
        resp = client.delete(f"/api/v1/collector/accounts/{account.id}")
        assert resp.status_code == 200
    ...
    def test_create_account_with_minimal_fields(self, client):
        """只填必填字段创建账号"""
        resp = client.post("/api/v1/collector/accounts", json={})
        assert resp.status_code == 200
...
class TestCollectorLogs:
    ...
    def test_list_logs_empty(self, client):
        """空日志列表 → 返回空数组"""
        resp = client.get("/api/v1/collector/logs")
        assert resp.status_code == 200
        assert resp.json() == []

    def test_clear_logs(self, client):
        """清空日志（空数据库也不会报错）"""
        resp = client.delete("/api/v1/collector/logs")
        assert resp.status_code == 200

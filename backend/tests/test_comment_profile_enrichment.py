...
def test_save_success_updates_global_cache_and_all_comment_copies(db, monkeypatch):
    room = LiveRoom(account_name="账号", anchor_name="主播", room_id_str="profile-cache-room")
    db.add(room)
    db.flush()
    ...
    db.flush()
    db.add_all([
        Comment(session_id=session.id, user_sec_uid="stable-user", comment_content="第一条"),
        Comment(session_id=session.id, user_sec_uid="stable-user", comment_content="第二条"),
    ])
    db.commit()
...
def test_force_refresh_still_respects_platform_retry_after(db, monkeypatch):
    room = LiveRoom(account_name="账号", anchor_name="主播", room_id_str="profile-retry-room")
    db.add(room)
    db.flush()
    session = LiveSession(room_id=room.id, live_status="ended")
    db.add(session)
    db.flush()
    db.add_all([
        Comment(session_id=session.id, user_sec_uid="cooldown-user", comment_content="想要资料"),
        CommentUserProfile(
...
def test_manager_sets_starting_scope_before_background_task_runs(monkeypatch):
    ...
    async def idle_run(session_id, force):

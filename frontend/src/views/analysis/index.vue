...
const coveredDomainCount = computed(() => workbench.value?.domain_coverage.filter(item => item.covered).length || 0);
  ...
  const metrics = [
    {
      key: 'knowledge',
      label: '知识价值',
      value: result.knowledge_value_score,
      max: 10,
    ...
    },
    {
      key: 'interaction',
      label: '问题互动',
      value: result.interactivity_score,
      max: 10,
    ...
    },
    {
      key: 'lead',
      label: '私信承接',
      value: result.lead_guidance_score,
      max: 10,
...
async function initializePage() {
  ...
  try {
    ...
    sessions.value = sortSessionsByLatest(unwrapServiceData(sessionsResponse, '直播场次读取失败'));
...
<template>
  <NSpace vertical :size="16" class="analysis-page business-page" :aria-busy="loading">
    ...
    <NCard v-else-if="!loadError || sessions.length" :bordered="false" class="card-wrapper session-control-card">
        ...
        <NTabPane name="overview" tab="复盘总览">
          <NSpin :show="contextLoading">
            <NSpace vertical :size="16">
              <template v-if="scoreResult">
                  ...
                  <NGi v-for="item in scoreMetrics" :key="item.key">
                    <div class="score-metric">
                      <div class="flex items-center justify-between gap-8px">
                        <div
                          class="size-32px flex-center rounded-9px bg-primary-50 text-primary dark:bg-primary-900/25"
                        >
                ...
                <NCard :bordered="false" class="action-plan-panel" size="small">
                  ...
                  <NEmpty v-else description="尚未生成下一场优化建议" class="py-28px">
                    <template #extra>
                      <NButton size="small" type="primary" @click="runOptimize">生成优化建议</NButton>
                    </template>
                  </NEmpty>
                </NCard>
              ...
              >
                <template #footer>
                  <NButton type="primary" :disabled="!analysisReady" :loading="actionBusy" @click="runFullReview">
                    开始完整复盘
                  </NButton>
                </template>
              </NResult>

...
- **话术工作台**：默认打开最近已有话术的真实场次，集中展示完整分段、全文、时间覆盖、AI 评分、内容分类和时间导航；任务状态卡可穿透查看主播、场次、失败原因、重试次数与已有分段，活跃任务每 5 秒静默刷新；未生成全文的场次使用正常空状态，不再误报 404。
...
7. 在详情的“跨场对比”选择真实基准场次，把确认的问题创建为整改任务，并在下一场验证。
8. 从时间轴收录表现明确的真实话术片段，人工确认后再进入话术资产库。
9. 在“知识库”点击“同步最近 20 场”，确认话术、评论和指标时间片数量，再使用问答输入业务问题。

## 场次复盘工作台

直播场次详情是 P0 与 P1 的统一入口，不再拆分独立 AI 页面：

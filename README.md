...
- **长场次可靠性**：完整话术使用 MySQL `LONGTEXT` 保存；手动重试会保留已完成分片并重置失败部分，Worker 数据库异常会先回滚再记录状态，避免一小时以上直播在最后合并全文时被误判失败。
...
知识库页面以自由问答为主、证据核验为辅。问答接口最多接收最近 8 条历史消息，省略式追问会继承上一轮问题和已引用场次；模型只能依据本次召回的话术、评论、分钟指标和整场知识作答，证据不足时返回无结果，不生成模拟业务数据。

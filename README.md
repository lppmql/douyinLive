...
`start.sh` 启动 MySQL、Redis、执行 Alembic 数据库迁移、启动后端和前端，后端会自动启动 FunASR。采集调度器由后端统一管理，不再额外启动第二个采集 Worker；FunASR 限制 2 核、1.8GB 内存，ffmpeg 单线程，ASR Worker 单任务并发且最多排队 5 场。

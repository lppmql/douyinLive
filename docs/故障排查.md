...
经营大屏直接读取 `live_sessions` 等真实业务源表。先确认页面日期和主播筛选，再检查组合接口：

```bash
curl -H "Authorization: Bearer <登录令牌>" \
  "http://localhost:8000/api/v1/dashboard/operations?start_date=2026-08-01&end_date=2026-08-30"
```

接口有数据而页面没有时，检查浏览器控制台和响应 `X-Trace-ID`；接口也为空时，到直播场次页确认对应场次已经采集入库。
...
| 症状 | 可能原因 | 处理 |
|------|----------|------|
| 点击“生成复盘”或离线终稿完成后没有创建剪辑 | AI 剪辑已改为纯手动，这是预期行为 | 在“AI 手动剪辑”页面选择真实场次并点击生成 |
| AI 复盘、问答或选段提示本地模型不可用 | Ollama 未启动，或项目模型尚未创建 | 打开 Ollama 应用，执行 `./scripts/setup_ollama_model.sh`，再运行 `make doctor` |
...
| 症状 | 可能原因 | 处理 |
|------|----------|------|
| 昨日场次没有自动入队 | 场次详情未完成、没有真实流源，或已经存在离线任务 | 先刷新数据采集，再到话术任务抽屉确认现有任务；自动范围为上海时区昨天、今天和全部直播中场次 |
| 修改线程参数后容器仍是旧值 | FunASR 容器没有重建 | 安全停止 ASR 后执行 `docker compose --profile funasr up -d --force-recreate funasr`，再用 `docker inspect douyin_live_funasr` 核对资源 |
| 增加线程后更慢或不稳定 | CPU 过度争用、内存上限过小，或模型线程不适合当前机器 | 先把 `FUNASR_MODEL_THREAD_NUM`、`FUNASR_IO_THREAD_NUM` 回退为 `1`；保留完成分片后重建容器，不要删除任务 |

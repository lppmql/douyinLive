...
ollama --version     # 本机模型运行时
```

> **AI 自动剪辑依赖**：成片字幕烧录需要带 libass 的 ffmpeg（服务器系统 ffmpeg 一般没有）。
> 部署时下载 evermeet.cx 静态版（macOS）或从源码编译带 libass 的版本（Linux），
> 放到项目 `.runtime/ffmpeg/ffmpeg`，或用环境变量 `CLIP_FFMPEG_BIN` 指定路径；
> 缺失时剪辑任务会报"找不到支持字幕烧录的 ffmpeg"。
...
配置完成后启动 Ollama，并在项目根目录创建项目模型；不再需要任何云端 AI API Key：

```bash
./scripts/setup_ollama_model.sh
```

生产机至少建议 16GB 内存；模型只监听 `127.0.0.1:11434`，不要把未鉴权的 Ollama 端口暴露到公网。
...
从 2026-08-30 之前的旧版本升级时，历史只读视图可能由 MySQL 高权限账号创建。若迁移提示需要 `SYSTEM_USER`，仅本次把 `DB_USER` 临时设为 `root`、把 `DB_PASSWORD` 临时设为 `.env` 中的 `MYSQL_ROOT_PASSWORD` 后执行 `alembic upgrade head`；完成后立即恢复普通应用账号，不要长期以 root 运行应用。
...
经营大屏已内置在前端，不需要部署额外可视化服务。运行状态通过 `/health`、响应 `X-Trace-ID` 和结构化日志排查。

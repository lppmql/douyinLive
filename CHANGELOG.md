...
### Fixed
- 公共场次选择器不再固定截断为最新 50 场；主播话术、AI 复盘、AI 剪辑和知识库改为每页 100 场的滚动加载，主播、日期与关键词筛选均可继续浏览全部历史结果。
- ASR 任务轮询更新下拉标签时不再重置菜单滚动位置，解决主播话术场次下拉框每隔数秒自动跳回顶部的问题。
- 头像代理继续限制为可信 HTTPS 图片域名，并补充抖音历史场次实际使用的火山 CDN 子域，避免展开历史场次时出现头像 400。

### Changed
- 公共场次与剪辑候选接口新增向后兼容的 `offset` 参数；旧场次深链作为额外回显项，不占用分页名额。分页边界见 ADR 0053。

### Verification
- 完整 `make check` 通过 571 项后端测试、60.97% 覆盖率、前端类型与全量 Lint、生产构建、Alembic 和 Docker 配置；头像白名单补丁另通过 21 项相关测试及 Ruff。
- 真实库 2632 场验证前两页各 100 场、无重复且与倒序前 200 场完全连续；Playwright 13 项页面场景均通过，专项确认 `offset=100` 被触发且追加后滚动位置大于 0。
...
- 客资增量同步对单条异常超长抖音号做字段级安全降级，不再误报整页协议格式错误和 502；真实客资仍保存并推进游标。
...
- 真实客资游标验收新增 75 条并全部完成场次归属，1 条异常超长抖音号被安全置空，整条客资保留且游标正常推进，未再出现 502。
- 已登录浏览器验收数据采集页正常渲染，AI 复盘卡显示后台自动；ASR 排队期间正确待命，页面与相关接口无 500 或错误遮罩。

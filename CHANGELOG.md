...
### Changed
- 评论用户真实头像和公开抖音号改为后台常驻自动补全：应用启动后每分钟发现新增或缓存到期用户，无需在场次详情页手动触发。
- 自动模式每轮最多处理 30 人，继续沿用专用 Cookie、固定指纹、1.5 秒请求间隔、缓存、失败退避和平台风控冷却；专用 Cookie 不可用时保持待命，不阻断评论与场次采集。
- 回访用户在新场次产生评论时优先复用数据库中已经验证的头像和公开抖音号，不重复请求平台，也不会因缓存命中留下页面覆盖缺口。
- 场次详情移除“补全本场用户资料”按钮，复用 Naive UI 标签和进度条展示后台自动获取状态；架构与降级边界见 ADR 0049。

### Fixed
- 统一 AI 复盘在本地 Ollama 重启或短暂断连时自动重建连接并按 2 秒、5 秒退避重试；恢复失败时保存明确原因并向前端返回 503，不再只显示笼统的“AI分析失败”。

### Verification
- 完整检查通过 553 项后端测试、60.17% 覆盖率、前端类型与全量 Lint、生产构建、Alembic 和 Docker 配置；真实专用 Cookie 验收结果记录在本次交付说明中。

## [2026-08-31]

### Fixed
- ASR 实时尾段不足 120 秒但已经产出话术时，封存已识别内容并从真实结束点续接，不再删除有效文字后重复坏尾段；实时流失效、连接中断和离线失败任务按任务级次数上限自动续接，只重置失败分片。
- 离线任务在实时通道占用且已有实时任务排队时不再反复领取和礼让；异常历史分片在实际领取后按真实时长收敛。

### Added
- 离线终稿完成后使用本机 Ollama/Qwen 做低优先级保守纠错，原始 FunASR 文本永久保留；数字、中文金额、百分比、面积、地区和可靠词典术语均经过防篡改校验。
- 纠错按批次事务提交并支持断点续跑；任何 ASR 任务到来时取消模型请求并礼让，单批最多等待 120 秒，不与 FunASR 争抢本机模型资源。

### Changed
- 采集日志中的遗留“旧后处理”统一改为“本地话术纠错”；架构、失败降级和回滚边界见 ADR 0048。

### Verification
- 新增并通过尾段文字保留、失败分片续接、中文事实保护、纠错事务回滚、断点组批和异步本地模型协议测试；完整项目检查与真实近期任务复核记录在本次交付说明中。

## [2026-08-30]

### Added
- 新增 SoybeanAdmin 原生经营大屏，复用公共主播与北京时间日期筛选，一次展示真实场次总体指标、经营趋势、留资漏斗、主播排行和最近场次明细。
- 新增 `/api/v1/dashboard/operations` 组合接口；原有汇总接口同步支持稳定主播键筛选，并补充真实业务表集成测试。

### Removed
- 退役 DataEase、Prometheus 和 Grafana：删除容器编排、配置、后端同步链路、指标导出接口、前端 iframe 与相关环境变量和依赖。
- 新增不可逆迁移 `h9d0e1f2a3b4`，删除 12 个旧只读视图和 7 张派生汇总表、取消遗留同步任务；`live_sessions`、评论、客资、话术、复盘等真实业务源数据保持不变。

### Changed
- 启动模式收敛为 `lite` 与 `standard`；轻量可观测性保留健康检查、Trace ID、结构化日志、任务状态与 AI 调用追踪。
- 部署、开发、故障排查、验收和数据库字典统一更新为原生大屏与核心服务口径；架构取舍和回滚边界见 ADR 0047。

### Verification
- 高风险检查覆盖完整后端测试、59.91% 覆盖率、前端类型与全量 Lint、生产构建、Alembic 和 Docker 配置；完整后端阶段发现 2 项旧汇总夹具缺少新增漏斗字段，修复与代码审查调整后相关 14 项测试和 Ruff 通过，未重复已经完成的 528 项测试。
- 真实业务库已先生成迁移前备份，再应用 `h9d0e1f2a3b4`；复查确认旧派生表/视图、独立数据库和账号均为 0，Alembic 位于唯一 head 且无待生成操作。
- 原生经营大屏 Playwright 冒烟通过，真实组合接口返回主播、趋势、漏斗和场次数据，页面无 iframe；旧容器、镜像、数据卷和运行目录完成实际清理。

## [2026-08-29]

### Added
- 新增公共场次选择组件，统一主播筛选、北京时间日期范围、远程关键词搜索、真实主播身份和旧场次直达回显；主播话术、AI 复盘、AI 剪辑和知识库四个页面共同使用。
- 新增真实主播筛选项和公共场次候选接口；剪辑候选接口同步支持主播与日期条件，同时保留已结束、详情完整及话术/成片统计门槛。

### Changed
- 知识库保留全局问答，并新增正式的单场检索边界：场次 ID 同时约束关键词、向量和 MySQL 补齐结果，不再只依靠提问文案限制来源。
- 知识库对话保存可空的场次归属；切换场次开启新对话，打开历史对话恢复原场次，旧对话继续按全知识库处理。
- 公共组件复用 Naive UI、VueUse 和现有 `AnchorIdentity`，未增加第三方依赖；架构与兼容取舍见 ADR 0046。
- 主播筛选改用 `douyin_uid`、抖音号、“直播间 + 主播快照”逐级降级的身份键，避免同名或同直播间轮班主播合并，空身份脏快照不生成选项；公共筛选、历史对话和流式问答增加过期响应保护，切换场次会真实取消旧 SSE 请求，主播选项读取失败不再阻断页面主体。

### Migration
- 新增 `g8c9d0e1f2a3`，为 `conversations` 增加可空 `session_id` 外键和索引；历史对话不猜测归属，删除场次时保留对话并将范围置空。

### Verification
- 高风险检查的代码与构建阶段通过：539 项 pytest、覆盖率 59.91%，前端类型检查、Oxlint/ESLint 和生产构建均通过；`docker compose config -q` 通过。
- 公共场次/知识库新增契约测试 12 项和相关 Ruff 通过，完整前端 `vue-tsc` 与相关文件的 Oxlint/ESLint 通过。Alembic 已确认新迁移是唯一 head，并在隔离的 MySQL 8 空库上通过全量 `upgrade head`、`alembic check`、新迁移 `downgrade`、再次 `upgrade` 和复查；未修改或重置现有业务库。

## [2026-08-28]

### Fixed
- 历史详情时间校验失败、平台暂时返回空数据改为可重试；兼容旧版两类误标为不可用的记录，仍保留防串场校验。
- 详情状态标签及筛选统一改为“详情待重试 / 详情暂不可用”，不再误显示“平台不可回放”。页面时间尚未到达时最多额外等 3 秒，正常场次不增加延迟。
- 大屏显示的省略号/星号主播名称不再覆盖已保存的完整名称；真实定点验收中发现的 #2529 名称截断一并恢复。

### Changed
- 失败重试每次刷新最多 10 场、冷却 30 分钟，最久未尝试优先；首次采集不减量，未处理记录计入剩余数量。新增内部定点补采范围，空范围不会触发全量。
- 保持现有数据库结构、DataEase 映射和 ASR 完整性门槛；决策与真实场次依据见 ADR 0045。

### Verification
- 一次完整 `make check` 通过：524 项 pytest、覆盖率 59.36%，前端类型检查、Lint、构建、Alembic 和 Docker 配置均通过。后续名称保护补跑 46 项相关测试及 Ruff，全部通过，未重复全量检查。
- 使用项目采集号和已保存指纹定点补齐 #2529：检查 1 场、成功 1 场、失败 0 场，状态恢复为 `complete`、旧错误清空，入库 81 条指标、45 条评论、36 项画像；未发起全量历史重采。
...
### Changed
- 检查按风险分级：新增普通文档、单后端模块、单前端模块的快速入口，局部 pytest 不再触发全局覆盖率；高风险 `make check` 和代码类 CI 保留 45% 覆盖率及完整校验。
- CI 对普通说明文档仅做差异检查，行业知识、代码与配置仍执行全量；API 类型校验并入后端任务，复用 Python 依赖，避免重复安装。同步测试 Agent 规则，已通过的检查不重复执行。
...
### Verification
- 检查轻量化：本地完整 `make check` 一次通过，503 项 pytest（新增 45 项分级回归测试）、覆盖率 58.27%，类型检查、Lint、构建、Alembic、Docker 均通过；CI YAML 结构及后端快速入口的缺参保护通过。未触发无关真实模型、长音频或采集验收。
